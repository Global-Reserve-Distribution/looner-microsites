...
export function InteractiveFlavorSelector({ products }: InteractiveFlavorSelectorProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-3">
        {flavors.map((flavor) => (
          <button
            ...
            className={`
              relative rounded-2xl p-4 text-center transition-all duration-200 hover:scale-105
              ${flavor.color} 
              ${selectedFlavor === flavor.id 
                ? 'ring-4 ring-cannabis-500 ring-offset-2 shadow-lg' 
                : 'hover:shadow-md'
              }
            `}
          >
            {/* Product Can Visual */}
            <div className="w-16 h-20 mx-auto mb-3 relative">
              {flavor.image ? (
                <img 
                  src={flavor.image} 
                  alt={flavor.name}
                ...
                <div className="w-full h-full bg-white/80 rounded-lg shadow-sm flex flex-col items-center justify-center">
                  <div className="text-xs font-bold text-gray-800 mb-1">LOONER</div>
                  <div className="text-[8px] text-cannabis-600">{flavor.thc}</div>
            ...
            <div className="text-[10px] text-cannabis-600 font-medium">
              {flavor.thc} THC
            </div>
            
            {/* Selection Indicator */}
            {selectedFlavor === flavor.id && (
              <div className="absolute -top-2 -right-2 w-6 h-6 bg-cannabis-600 rounded-full flex items-center justify-center">
      ...
      {selectedFlavor && (
        <div className="p-4 bg-cannabis-50 rounded-xl border border-cannabis-200">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-cannabis-100 rounded-full flex items-center justify-center">
              <svg className="w-6 h-6 text-cannabis-600" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
              </svg>
            </div>
            <div>
              <div className="font-semibold text-cannabis-800">
                {flavors.find(f => f.id === selectedFlavor)?.name} Selected
              </div>
              <div className="text-sm text-cannabis-600">
                {flavors.find(f => f.id === selectedFlavor)?.thc} THC content
              </div>
            </div>
          </div>
        </div>
